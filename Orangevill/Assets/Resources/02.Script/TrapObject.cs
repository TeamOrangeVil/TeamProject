...
public class TrapObject : MonoBehaviour
{
        ...
        if (!isActing)
        {
            ...
            switch (gameObject.name)
            {
                ...
                case TrapNames.mobleFloor://발판
                    isActing = true;
                    //objRigidbody.AddForce((Vector2.left * 2.4f * head) + Vector2.up * 4f, ForceMode2D.Impulse);
                    if (!isSaved && GameManager.Instance.nowSave) { GameManager.Instance.PushIntoStack(this); isSaved = true; }// 중복세이브 방지
    ...
    IEnumerator ShakeFloor()//흔들리는 발판
    {
        float temp = 1;
        int shakeTime = 0;
        float tempZRot = transform.rotation.z;
        while (shakeTime <= 50)
        {
            Debug.Log("실행실행");
            ...
            transform.rotation = Quaternion.Euler(0, 0, temp * Time.deltaTime * 3.0f);
            shakeTime++;
            yield return null;
        }
        StartCoroutine(FallDownFloor());
