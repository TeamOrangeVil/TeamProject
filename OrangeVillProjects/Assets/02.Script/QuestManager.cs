...
public class QuestManager : MonoBehaviour {
    ...
    public void LoadQuest()//q번째 퀘스트 정보를 불러옵니다.
    {
        ...
        if (questInfo == null || questProgress == null)
            Debug.Log("아 없잖아;");
        Debug.Log("데이터로드 실행");
        questInfo = xmlParsing.QuestInfoRead(Application.streamingAssetsPath + XmlConstancts.QUESTINFOXML, questCount);
        questProgress = xmlParsing.QuestProgressRead(Application.streamingAssetsPath + XmlConstancts.QUESTDIALOGXML,
    ...
    {
        if (questInfo.req_Item == itemName)//주운 아이템이 퀘템이면
        {
            ClearToQuest++;
