...
{
    private LineRenderer lineRenderer;          // 라인렌더러 컴포넌트
    public GameObject targetobj;                // 로프 손잡이 부분
    public Vector3 center = Vector3.zero;       // 타겟과 자신과의 중간거리
    public Vector3 theSlerp = Vector3.zero;     // 각각의 라인렌더러에 넣을 좌표
    public Vector3 RelCenter = Vector3.zero;
    public Vector3 aimRelCenter = Vector3.zero;
    ...
    void Update()
    {
        ...
        RelCenter = transform.position - center;
        aimRelCenter = targetobj.transform.position - center;
        
        for (float i = 0, interval = -0.053f; interval < 1.0f; i++)
        {
            theSlerp = Vector3.Slerp(RelCenter, aimRelCenter, interval += 0.05f);
            lineRenderer.SetPosition((int)i, theSlerp + center);
        }
        //}
       // else
        //{
            //lineRenderer.SetPosition(0, CharacterController2D.Instance.transform.position + Vector3.up * 1.5f);
           // for (float i = 0, interval = 1.0f; interval > 0.0f;)
            //{
